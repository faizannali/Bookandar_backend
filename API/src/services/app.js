...
app.post('/login/userCheck', (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        connection.query('SELECT fullName,phoneNumber,password from userinfo where phoneNumber = ? and password = ?', params, (err, data) => {
...
app.post('/showMyAds', (req, res) => {
    ...
    pool.getConnection((err, connection) => {
        ...
        console.log(params);
        connection.query('Select * From user_ads Where phoneNumber = ?', params, (err, rows) => {
            connection.release() // return the connection to pool
            if (!err) {
...
app.post('/deleteMyAds', (req, res) => {
    ...
    pool.getConnection((err, connection) => {
        ...
        console.log(params);
        connection.query('Delete From user_ads Where phoneNumber = ? and orderId = ?', params, (err, rows) => {
            connection.release() // return the connection to pool
            if (!err) {
...
app.post('/updateMyAds', (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        params = [req.body.bookName, req.body.bookDesc, req.body.bookGenre, req.body.bookLocation, req.body.bookArea, req.body.bookPrice, req.body.orderId]
        console.log(params);
        connection.query('UPDATE user_ads SET bookName = ?, bookDesc= ?,bookGenre= ?,bookLocation=?,bookArea=?,bookPrice=? WHERE orderId = ?;', params, (err, rows) => {
            connection.release() // return the connection to pool
            if (err) {
...
app.post('/books/updateUserProfile', (req, res) => {
    pool.getConnection((err, connection) => {
        ...
        console.log(params);
        connection.query('UPDATE userInfo SET email= ?, area=? WHERE phoneNumber = ?;', params, (err, rows) => {
            connection.release() // return the connection to pool
            if (err) {
...
app.post('/profileInfo', (req, res) => {
    ...
    pool.getConnection((err, connection) => {
        ...
        const params = [req.body.phoneNumber];
        console.log(params);
        connection.query('Select * From userinfo Where phoneNumber = ?', params, (err, rows) => {
            connection.release() // return the connection to pool
            if (!err) {
                res.send(rows[0]);
            } else {
